    ...
    companion object {
        ...
        fun getInstance(context: Context) = instance ?: synchronized(this) {
            instance ?: PlacepicAuthRepository().also {
                instance = it
                shardPreference = context.getSharedPreferences(PLACEPIC_AUTH,Context.MODE_PRIVATE)
            }
        }
    }

    private var editer = shardPreference.edit()

    val userToken : String?
        get() = shardPreference.getString(PLACEPIC_AUTH,"")

    val groupId : Int?
        get() = shardPreference.getInt(PLACEPIC_AUTH,0)
